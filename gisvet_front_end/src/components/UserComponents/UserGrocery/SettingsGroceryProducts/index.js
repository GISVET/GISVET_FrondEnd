...
export default function SettingsAdminProducts({ sendProducts, setSendProducts }) {
    ...
    const showAddMark = async(event) => {
        ...
        setShowModal(true);
        setchildModal( <
            AddMark onClose = { handleCloseModal }
            onSubmit = { onsubmitAddMark }
            />
        );
    };
    ...
    const ShowAddItemProduct = async(event) => {
        ...
        setShowModal(true);
        setchildModal( <
            AddItemProduct onClose = { handleCloseModal }
    ...
    const onsubmitAddMark = (dataForm) => {
      return addMark(dataForm).then(res => {
            setchildModal( < MessageConfirm onClose = { handleCloseModal }
  ...
  if (!activeMenu) {
    return ( <div className = { styles.options_admin } >
        <input className = { styles.settings_clicked }
          type = "image"
    ...
    return ( <>
        <div className = { styles.options_admin_visible } >
          <input className = { styles.settings_show }
            type = "image"
            onClick = { setVisibleMenu }
            src = { icon_Settings }
            width = "45"
        ...
        </div> {showModal && < Modal > { childModal } </Modal>} 
        </>
        );
    }
}